...
namespace SmartLib
{
    internal static class DeepClonerMsilGenerator
    {
        ...
        private static void GenerateProcessMethod(ILGenerator il, Type type, bool unboxStruct)
        {
            ...
            {
                var constructor =
                    type.GetConstructor(BindingFlags.NonPublic | BindingFlags.Public | BindingFlags.Instance, null,
            ...
            foreach (var fieldInfo in fi)
                ...
                else
                {
                    ...
                    var methodInfo = fieldInfo.FieldType.IsValueType
                        ? typeof(DeepClonerGenerator).GetMethod("CloneStructInternal",
                                BindingFlags.NonPublic | BindingFlags.Static)
                            .MakeGenericMethod(fieldInfo.FieldType)
                        : typeof(DeepClonerGenerator).GetMethod("CloneClassInternal",
                            BindingFlags.NonPublic | BindingFlags.Static);
        ...
        private static void GenerateProcessArrayMethod(ILGenerator il, Type type)
        {
            ...
            if (rank != 1 || type != elementType.MakeArrayType())
            {
                ...
                if (rank == 2 && type == elementType.MakeArrayType(2))
                    ...
                    methodInfo = typeof(DeepClonerGenerator).GetMethod("Clone2DimArrayInternal",
                            BindingFlags.NonPublic | BindingFlags.Static)
                        .MakeGenericMethod(elementType);
                else
                    methodInfo = typeof(DeepClonerGenerator).GetMethod("CloneAbstractArrayInternal",
                        BindingFlags.NonPublic | BindingFlags.Static);
            ...
            il.Emit(OpCodes.Call, type.GetProperty("Length")
                .GetGetMethod());
            il.Emit(OpCodes.Dup);
            ...
            else
            {
                var methodInfo = elementType.IsValueType
                    ? typeof(DeepClonerGenerator)
                        .GetMethod("CloneStructInternal", BindingFlags.NonPublic | BindingFlags.Static)
                        .MakeGenericMethod(elementType)
                    : typeof(DeepClonerGenerator).GetMethod("CloneClassInternal",
                        BindingFlags.NonPublic | BindingFlags.Static);
                ...
                if (type.IsValueType)
                {
                    ...
                    il.Emit(OpCodes.Call, typeof(DeepClonerGenerator)
                        .GetMethod("GetClonerForValueType", BindingFlags.NonPublic | BindingFlags.Static)
                        .MakeGenericMethod(elementType));
