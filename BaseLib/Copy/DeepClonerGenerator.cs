...
namespace SmartLib
{
    internal static class DeepClonerGenerator
    {
        ...
        private static object CloneClassRoot(object obj)
        {
            ...
            var cloner =
                (Func<object, DeepCloneState, object>)DeepClonerCache.GetOrAddClass(obj.GetType(),
        ...
        internal static Array CloneAbstractArrayInternal(Array obj, DeepCloneState state)
        {
            ...
            var lengths = Enumerable.Range(0, rank)
                .Select(obj.GetLength)
                .ToArray();

            var lowerBounds = Enumerable.Range(0, rank)
                .Select(obj.GetLowerBound)
                .ToArray();
            var idxes = Enumerable.Range(0, rank)
                .Select(obj.GetLowerBound)
                .ToArray();
        ...
        {
            return (Func<T, DeepCloneState, T>)DeepClonerCache.GetOrAddStructAsObject(typeof(T),
                t => GenerateCloner(t, false));
        }
        ...
        public static object CloneObjectTo(object objFrom, object objTo, bool isDeep)
        {
            ...
            if (!type.IsInstanceOfType(objTo))
                throw new InvalidOperationException(
                    "From object should be derived from From object, but From object has type " + objFrom.GetType()
                        .FullName + " and to " + objTo.GetType()
                        .FullName);
            ...
            var cloner = (Func<object, object, DeepCloneState, object>)(isDeep
                ...
                : DeepClonerCache.GetOrAddShallowClassTo(type,
                    t => ClonerToExprGenerator.GenerateClonerInternal(t, false)));
