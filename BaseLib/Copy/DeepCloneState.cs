        ...
        private class MiniDictionary
        {
            ...
            public object FindEntry(object key)
            {
                if (_buckets != null)
                {
                    ...
                    for (var i = _buckets[hashCode % _buckets.Length];
                        i >= 0;
                        i = entries1[i]
                            .Next)
                        if (entries1[i]
                                .HashCode == hashCode && ReferenceEquals(entries1[i]
                                .Key, key))
                            return entries1[i]
                                .Value;
                }
            ...
            private static int ExpandPrime(int oldSize)
            {
                ...
                if ((uint)newSize > 0x7FEFFFFD && 0x7FEFFFFD > oldSize) return 0x7FEFFFFD;
