...
namespace SmartVEye
{
    ...
    /// IO工具类 第一代例程函数挪用
    /// </summary>
    unsafe public class IOHelperOld1
    {
        #region GPIO配置
        ///
        /// *******************************************************
        ...
        unsafe private static Response SetBitOn1()
        {
            ...
            GpioData = 0x00000000;
            GetIoPortValue(GPIO_BASE_ADDRESS + R_PCH_GPIO_LVL2, &GpioData, IO_PROTOCOL_WIDTH.IoWidthUint32);
            GpioData |= 0x00000002;
            RetStatus = SetIoPortValue(GPIO_BASE_ADDRESS + R_PCH_GPIO_LVL2, GpioData, IO_PROTOCOL_WIDTH.IoWidthUint32);
            if (RetStatus != 0)
            {
